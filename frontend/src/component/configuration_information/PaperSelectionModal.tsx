...
const PaperSelectionModal = ({ open, onClose }: { open: boolean; onClose: () => void }) => {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      setSelectedRowIndex((prev) => {
        ...
        if (key === "ArrowDown") return prev === null ? 0 : Math.min(prev + 1, len - 1);
        ...
        footer={
          <div className="flex justify-around w-full">
            {["実行", "削除", "閉じる"].map((label) => (
              <Button
                key={label}
                onClick={label === "閉じる" || label === "実行" ? onClose : undefined}
        ...
        <ConfigProvider locale={jaJP}>
          ...
          <Row gutter={16} className="mb-4">
            <Col span={12} className="flex items-center">
              ...
              <DatePicker className="flex-1" format="YYYY/MM/DD" value={dayjs("2010-12-30")} />
            </Col>
            <Col span={12} className="flex items-center">
              <span className="bg-[#D9D9D9] font-bold text-sm text-black px-2 py-1 mr-2" >
                更新日
              </span>
              <DatePicker className="flex-1" format="YYYY/MM/DD" value={dayjs("2015-12-30")} />
            </Col>
          </Row>
