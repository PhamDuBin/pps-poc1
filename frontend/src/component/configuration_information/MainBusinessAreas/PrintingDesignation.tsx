...
const printingButtons = [
  ...
  "営業担当","集金担当","点検担当","検針担当","配送担当","保安担当","集金方法","請求書発行区分",
];

const printingOrderOptions = ["顧客コード","五十音順","検針順","営業順","集金順","配送順","点検順","検索１順","検索２順"];
const detailOptions = ["全明細","集金明細","自振明細"];
const taxTypeOptions = ["外税","内税"];
const taxCollectOptions = ["する","しない"];
const printItemOptions = ["事業所名","ガス料金名称","電話番号","領収額","振込先","お買い上げ先"];
const memoOptions = ["伝票メモ","ポイント"];
const memoRadioOptions = ["割引（割引対象外顧客の場合は伝票メモを印字）","割引（割引対象外顧客の場合はポイントを印字）"];
const parentChildOptions = ["請求親子取りまとめて発行する。","個別に発行する。"];
const addressOptions = ["宛先登録内容を印字しない","宛先登録内容を印字する"];
const detailOrderOptions = ["日付順","大分類・商品コード・日付順","大分類・日付順"];
    ...
    <div className="flex gap-3">
      {options.map((opt: string, idx: number) => (
        <label key={idx} className="flex items-center gap-2">
          <input type="radio" name={name} value={idx} checked={selected === String(idx)} onChange={(e) => onChange(e.target.value)} />
          {opt}
        </label>
      ))}
    </div>
        ...
        <div className="flex gap-3 flex-wrap absolute">
          {options.map((opt: string, idx: number) => (
            <label key={idx} className="flex items-center gap-3">
              <input className="w-6 h-6" type="checkbox" checked={selected.includes(String(idx))} onChange={(e) => {
                if (e.target.checked) onChange([...selected, String(idx)]);
                else onChange(selected.filter((i: string) => i !== String(idx)));
              }} />
              {opt}
            </label>
          ))}
    ...
    <div className="p-2 border border-black mt-2">
      ...
      <div className="mt-4 flex flex-col gap-2">
        <div className="flex justify-between">
          <SelectInput label="印刷順" value={selectedOrder} options={printingOrderOptions} onChange={setSelectedOrder} />
          <RadioGroup label="請求親子" name="parentChild" options={parentChildOptions} selected={selectedParentChild} onChange={setSelectedParentChild} />
        </div>
        <div className="flex justify-between">
          <SelectInput label="自振顧客明細" value={selectedDetail} options={detailOptions} onChange={setSelectedDetail} />
          <RadioGroup label="宛先印字" name="address" options={addressOptions} selected={selectedAddress} onChange={setSelectedAddress} />
        </div>
        <div className="flex justify-between">
          <SelectInput label="鑑部税表現" value={selectedTaxType} options={taxTypeOptions} onChange={setSelectedTaxType} />
          <SelectInput label="明細順" value={selectedDetailOrder} options={detailOrderOptions} onChange={setSelectedDetailOrder} />
        </div>
        <SelectInput label="税取りまとめ" value={selectedTaxCollect} options={taxCollectOptions} onChange={setSelectedTaxCollect} />
        <CheckboxGroup label="印刷項目" options={printItemOptions} selected={selectedPrintItems} onChange={setSelectedPrintItems} />
        <div className="flex gap-3">
          <SelectInput label="伝票メモ" value={selectedMemo} options={memoOptions} onChange={setSelectedMemo} />
          <RadioGroup label="" name="memoRadio" options={memoRadioOptions} selected={selectedMemoRadio} onChange={setSelectedMemoRadio} gap="gap-1" />
        </div>
        <div className="flex justify-between">
