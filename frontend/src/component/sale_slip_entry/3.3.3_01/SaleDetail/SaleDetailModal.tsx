...
const SaleDetailModal: React.FC<SaleDetailModalProps> = ({
  ...
  switch (categoryName) {
    case "1.売上":
      SelectedEntry = (
        <SaleDetailEntry1 onChange={handleChange} formData={formData} />
      );
      break;
    case "2.直送売上":
      SelectedEntry = (
        <SaleDetailEntry2 onChange={handleChange} formData={formData} />
      );
      break;
    case "3.売上値引":
      SelectedEntry = (
        <SaleDetailEntry3 onChange={handleChange} formData={formData} />
      );
      break;
    case "4.返品":
      SelectedEntry = (
        <SaleDetailEntry4 onChange={handleChange} formData={formData} />
      );
      break;
    case "5.経費":
      SelectedEntry = (
        <SaleDetailEntry5 onChange={handleChange} formData={formData} />
      );
      break;
    case "6.資産":
      SelectedEntry = (
        <SaleDetailEntry6 onChange={handleChange} formData={formData} />
      );
      break;
    case "7.消費税":
      SelectedEntry = (
        <SaleDetailEntry7 onChange={handleChange} formData={formData} />
      );
      break;
  }
  ...
  const handleNext = useCallback(() => {
    ...
    onNext(data);
  }, [formData, categoryName, onNext, rowEdit]);
  ...
  return (
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 ">
      ...
      <div className="bg-gray-100 rounded-lg shadow-xl p-5 border border-gray-300 w-[840px] h-[650px]">
        ...
        {categoryName !== "7.消費税" && (
          <div className="mt-5">
            <SaleDetailSelector onReSearch={onClose} />
          </div>
        )}

        <div className="my-5">{SelectedEntry}</div>
